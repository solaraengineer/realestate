...
=======
    ...
    if (btnSetVP) {
      btnSetVP.addEventListener('click', async () => {
        ...
        try {
          const resp = await fetch('/api/viewpoints/save/', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
    ...
    if (btnWatch) {
      btnWatch.addEventListener('click', async () => {
        ...
        try {
          const resp = await fetch('/api/observations/save/', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'X-CSRFToken': getCookie('csrftoken'),
                  ...
                  fetch(`/api/users/search/?q=&id=${sellerId}`, { credentials: 'same-origin' })
                    ...
                    .then(data => {
                      if (data.users && data.users.length > 0) {
                        window.ChatPanel.openChat(sellerId, data.users[0].username);
                      } else {
